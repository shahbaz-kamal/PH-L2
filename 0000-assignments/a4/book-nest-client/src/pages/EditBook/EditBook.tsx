import Heading from "../../components/Shared/Heading";
import { useForm, type SubmitHandler } from "react-hook-form";
import { Button } from "../../components/ui/button";
import { useParams } from "react-router";
import {
  useGetSingleBookQuery,
  useUpdateBookMutation,
} from "../../redux/api/bookApi";
import { useAppDispatch, useAppSelector } from "../../redux/hooks";
import { useEffect } from "react";
import { setEditPageData } from "../../redux/features/books/bookSlice";
import Swal from "sweetalert2";

const EditBook = () => {
  const dispatch = useAppDispatch();
  const editPageData = useAppSelector((state) => state.book.editPageData);
  const [updateBook] = useUpdateBookMutation();

  const { id } = useParams();
  const { data } = useGetSingleBookQuery(id || "");
  const form = useForm<FormField>({
    defaultValues: {
      title: "",
      author: "",
      genre: "",
      isbn: "",
      description: "",
      coverPage: "",
      price: 0,
      copies: 0,
      available: "AVAILABLE",
    },
  });

  // console.log(data);

  useEffect(() => {
    if (data?.data) {
      dispatch(setEditPageData({ editPageData: data.data }));
    }
  }, [data, dispatch]);

  console.log("from edit page", editPageData);
  type FormField = {
    _id?: string;
    title: string;
    author: string;
    genre: string;
    isbn: string | number;
    description: string;
    coverPage: string;
    price: number;
    copies: number;
    available: string | boolean;
  };

  const register = form.register;
  const handleSubmit = form.handleSubmit;
  const reset = form.reset;

  useEffect(() => {
    if (editPageData) {
      const formData = {
        ...editPageData,
        available:
          editPageData.copies === 0
            ? "NOT_AVAILABLE"
            : editPageData.available
            ? "AVAILABLE"
            : "NOT_AVAILABLE",
      };
      reset(formData);
    }
  }, [editPageData, reset]);
  const copiesValue = Number(form.watch("copies", editPageData?.copies || 0));

  useEffect(() => {
    if (copiesValue === 0) {
      form.setValue("available", "NOT_AVAILABLE");
    }
  }, [copiesValue, form]);

  const onSubmit: SubmitHandler<FormField> = async (data) => {
    const { _id, ...body } = data;
    console.log(_id);
    if (body.copies === 0) {
      body.available = false; // not available
    } else {
      body.available = body.available === "AVAILABLE" ? true : false;
    }

    const res = await updateBook({ id: id as string, body });
    console.log("from updaTE REs", res);

    if (res.data.success) {
      Swal.fire({
        title: "Good job!",
        text: "You updated The book info successfully",
        icon: "success",
      });
      console.log(res.data);
    }
  };

  return (
    <div className="w-11/12 md:w-10/12 mx-auto">
      <header>
        <Heading title="Edit  Book Information"></Heading>
        <title>Edit Book || BookNest</title>
      </header>
      <main>
        <form
          onSubmit={handleSubmit(onSubmit)}
          action=""
          className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-light-primary/10 dark:bg-dark-primary/5 p-6 md:p-10 rounded-2xl mb-10"
        >
          {/* title */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Title
            </label>
            <input
              defaultValue={editPageData?.title}
              {...register("title")}
              name="title"
              placeholder="Enter the title"
              type="text"
              className="w-full rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            />
          </div>
          {/* Author */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Author
            </label>
            <input
              defaultValue={editPageData?.author}
              {...register("author")}
              placeholder="Enter the author's name"
              type="text"
              className="w-full rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            />
          </div>
          {/* Genre */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Genre
            </label>
            <select
              {...register("genre")}
              defaultValue={editPageData?.genre}
              name="genre"
              className="w-full rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            >
              <option value="FANTASY">FANTASY</option>
              <option value="BIOGRAPHY">BIOGRAPHY</option>
              <option value="HISTORY">HISTORY</option>
              <option value="SCIENCE">SCIENCE</option>
              <option value="NON_FICTION">NON_FICTION</option>
              <option value="FICTION">FICTION</option>
            </select>
          </div>
          {/* ISBN */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              ISBN
            </label>
            <input
              {...register("isbn")}
              name="isbn"
              placeholder="Enter ISBN number"
              type="text"
              className="w-full rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            />
          </div>
          {/* Description */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Description
            </label>
            <textarea
              {...register("description")}
              name="description"
              placeholder="Enter book description"
              rows={1}
              className="w-full rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            ></textarea>
          </div>
          {/* Coverpage image url */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Cover Image URL
            </label>
            <input
              {...register("coverPage")}
              name="coverPage"
              placeholder="Enter the image url"
              type="url"
              className="w-full rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            />
          </div>
          {/* Price */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Price
            </label>
            <input
              {...register("price")}
              name="price"
              placeholder="Enter book price"
              type="number"
              className="w-full rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            />
          </div>
          {/* Copies */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Copies
            </label>
            <input
              {...register("copies")}
              name="copies"
              placeholder="Enter available copies"
              type="number"
              className="w-full rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            />
          </div>
          {/* Availability */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Availability
            </label>
            <select
              {...register("available")}
              disabled={copiesValue === 0}
              className="w-full  md:col-span-2 rounded-xl border border-light-primary/30 dark:border-dark-primary/30 bg-white dark:bg-dark-background px-4 py-3 text-light-text dark:text-dark-text placeholder-gray-400 dark:placeholder-gray-500 focus:border-light-primary dark:focus:border-dark-primary focus:ring-2 focus:ring-light-primary/50 dark:focus:ring-dark-primary/50 outline-none transition-all"
            >
              <option value="AVAILABLE">AVAILABLE</option>
              <option value="NOT_AVAILABLE">NOT AVAILABLE</option>
            </select>
          </div>
          <div className="flex flex-col  md:col-span-2 gap-2">
            {/* <label className="text-sm font-medium text-light-text dark:text-dark-text  md:text-lg">
              Availability
            </label> */}
            <Button
              type="submit"
              {...register("available")}
              name="available"
              className="w-full rounded-md bg-light-primary dark:bg-dark-primary hover:bg-light-primary/80 dark:hover:bg-dark-primary/80 px-4 py-3 text-white text-lg font-semibold  outline-none transition-all"
            >
              Submit
            </Button>
          </div>
         
        </form>
      </main>
    </div>
  );
};

export default EditBook;
